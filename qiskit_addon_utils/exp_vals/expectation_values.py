...
def expectation_values(
    ...
    """
        Uses data in `bool_array`, acquired with measurement bases as ordered in keys of `basis_dict`, to compute
        observables encoded in values of `basis_dict`.
    ...
    if len(basis_dict) != bool_array.shape[meas_basis_axis]:
        raise ValueError(f"{len(basis_dict) =} does not match {bool_array.shape[meas_basis_axis] =}.")
    ...
    for i, v in enumerate(basis_dict.values()):
        ...
        if len(v) != num_observables:
            raise ValueError(f"Entry 0 in `basis_dict` indicates {num_observables} observables, but entry {i} indicates {len(v)} observables.")
    ...
    bool_array, basis_dict, num_shots_kept = apply_postselect_mask(bool_array, basis_dict, postselect_mask)
    ...
    for meas_basis_idx, (_, diagonal_observables) in enumerate(basis_dict.items()):
        ...
        vars = gamma**2 * np.sum(vars * weights**2, axis=avg_axis)

        mean_each_observable += means
        var_each_observable += vars
